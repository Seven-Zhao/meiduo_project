...
class SKUSpecification(BaseModel):
    ...
    def option_value(self):
        """
        显式地获取 option 的 value，
        """
        # 这里的类型提示 (self.option: SpecificationOption) 是关键
        # 它告诉 PyCharm，当访问 self.option 时，它是一个 SpecificationOption 实例
        option_instance = cast(SpecificationOption, self.option)
        return option_instance.value if option_instance else None

    def __str__(self):
        # return '%s: %s - %s' % (self.sku, self.spec.name, self.option.value)
        # 在 __str__ 方法中也可以直接使用 cast
        # option_instance = cast(SpecificationOption, self.option)
        # return f'{self.sku}: {self.spec.name} - {option_instance.value}'
        return f'{self.sku}: {self.spec.name} - {self.option_value}'
